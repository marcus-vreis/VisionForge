...
## ADR-090 — The greeting takes its name from the OS

**Date:** 2026-08-11
**Status:** Accepted — shipped 2026-08-11

**Context:** a welcome line in the top corner, `Bem-vindo, Marcus`, played once
when the page opens.

**Where the name comes from is the only real decision here.** VisionForge runs
locally, for one researcher, on their own machine. The OS already knows who is
sitting at it, so `/api/health` reports `getpass.getuser()` and the banner uses
it. The alternative — a settings field, or a prompt on first load — would be
inventing a form to ask a question that is already answered, and a prompt before
the user has done anything is the one people dismiss.

**Account names are not display names.** A domain prefix (`LAB\marcus`), a
dotted `first.last`, an email — all get reduced to the first token, capitalised.
A name with no letters in it is dropped entirely: `Bem-vindo, 12345` reads like
a database record, not a hello, so it falls back to a plain `Bem-vindo`.

**Once per session, not once per page load.** A reload in the middle of work is
not an arrival, and replaying the animation each time would turn a greeting into
a flicker people wait out. The mark lives in `sessionStorage`; when storage is
unavailable or throws, it greets rather than staying silent — the failure mode
of a greeting should be showing up, not vanishing.

**It leaves on its own and never asks for a click.** A dismissable banner is a
decision imposed about something that asked nothing of the reader. It fades
after roughly four seconds and is `pointer-events: none` throughout, so it can
never intercept a click meant for the app behind it.

**`prefers-reduced-motion` removes the movement, not the greeting.** Someone who
set that preference asked not to be moved; they did not ask to be ignored.

**A failed request renders nothing.** Greeting a stranger is worse than not
greeting at all, and a welcome line is not worth reporting an error over.
